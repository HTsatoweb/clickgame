...
(function () {
  ...
  var MAX_POINTS = 1400;
  var HIRES_POINTS = 500;
  var SAVE_EVERY_MS = 8000;
  ...
  var cfg = { precision: 40, intervalMs: 1000, showDots: false, overlay: true, maxPoints: 1200, paused: false, quality: "low", exactRaw: true };
  ...
  function loadCfg(){
    ...
    if(typeof o.intervalMs==="number") cfg.intervalMs=clamp(o.intervalMs,0,3600000);
    if(typeof o.showDots==="boolean") cfg.showDots=o.showDots;
    if(typeof o.overlay==="boolean") cfg.overlay=o.overlay;
    if(typeof o.maxPoints==="number") cfg.maxPoints=Math.max(0, Math.min(50000, o.maxPoints));
  ...
  function persistData(force){
    ...
    if(!saveJson(DATA_KEY,data)){
      ["energy","cps","am","ts","stars","clicks"].forEach(function(k){ if(data[k]&&data[k].length>1800) data[k]=downsample(data[k],1800); });
  ...
  function trimPush(target,t,yLog,maxN,rawStr){
    ...
    var rec={t:t,y:yLog}; if(rawStr!=null) rec.raw=String(rawStr);
    target.push(rec);
    if(!maxN) return;
